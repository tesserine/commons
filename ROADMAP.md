...
flowchart TB
    ...
    AGENTD162["agentd#162<br/>transcript path-contract fix — agentd reads runa's<br/>project-keyed nested event path (live tailer + audit finalize)<br/>(ADR-0020's named impl; plan surfaced a B4/CQ1 scope seam → contract regen)"]:::ready --> AGENTD122
...
- **agentd#162** — the transcript path-contract fix ADR-0020 named as its
  ...
  **both** the live tailer (agentd#122) and the audit finalize/manifest —
  ...
  together, and reconciling the v2 event schema. **Plan gate (2026-07-04): plan reviewed, sound but for one contract-scope
  correction.** The plan grounded that the live tailer is #122's code, absent
  from `main` (PR #161 closed) — so contract criteria B4/CQ1 over-scoped #162.
  Body corrected to split the both-readers obligation by branch: #162 owns the
  shared resolver + finalize reader (nested-growth read surface proven in-tree);
  #122 owns the live-follow proof on a verification-only rebase branch.
  Identity strings verified agentd-owned (clear the no-coupling guard). Next
  relay: regenerate `contract agentd#162` against the corrected body, then
  re-review the plan.
