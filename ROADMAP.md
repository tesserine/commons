...
flowchart TB
    ...
    subgraph Q7["🪄 QUEST · intent-entry surface — wish declares, run dispatches · agentd#160 ✓ (#165, #166 before M1 · #167–#168 after M1 + Radicle)"]
        direction TB
        ...
        q7_166["agentd#166<br/>desired-state wish copy<br/>→ before M1 (#67 DX-sufficiency)"]:::ready
    ...
    AGENTD122["agentd#122<br/>live session progress observation — code-complete<br/>#122-on-#162 verification build BUILT — PR #169 @ 08c9199, review-passed<br/>operator-evidence → #67 · merges on #67 session pass"]:::ready --> BO67
    RUNBOOK96["babbie-ops#96 ✓<br/>runbook: consolidated operator-eyes<br/>evidence lifecycle (single home; #67 proves)"]:::landed --> BO67
    BO67["babbie-ops#67 · CONSOLIDATED operator-eyes gate<br/>one agentd wish session on the #122-on-#162 verification build<br/>collects #162 (live+sealed) · #122 (live obs) · #67 (DX judgment)<br/>verification build READY (PR #169 @ 08c9199) → operator-runnable<br/>pass → merge #162 + #122, close #58"]:::ready --> M1INT
...
- **agentd#166** — re-derive the `wish` copy to desired-state framing (retiring the process-framed *"what do you wish the agent to do?"* prompt). **Pulled before M1** (operator decision 2026-07-06): a declarative surface asking for a procedure is a DX-sufficiency concern for the #67 acceptance. Copy-only — no schema or mechanism change; depends only on the landed spike agentd#160.
