...
Every element in ecosystem repos — every line of code, every test, every document, every abstraction — traces to a current need or gets removed. New work grounds in what it must enable before building. Existing work that no longer serves today's exigency is debt regardless of how well it functions.

## Context

The ecosystem learned early in its development that carrying historical artifacts, naming conventions, and structural assumptions from predecessor systems has a compounding cost. Debugging time spent untangling inherited assumptions exceeded the time a clean build would have taken. That experience established this as a foundational discipline.
